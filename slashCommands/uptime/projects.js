...
module.exports = {
  ...
  run: async (client, interaction, args) => {
    ...
    var menuEmoji = "<:GreenCheck:953157963525283870>";
    ...
    if (content.length === 0) {
      ...
      embed.setDescription(
        `*You don't have any projects Added.*\nAdd one by using: /moniter [project Url]`
      );
    } else {
    ...
    await interaction.member.user.send({ embeds: [embed] }).catch((err) => {
      ...
      if (err.message === "Cannot send messages to this user") {        
        return interaction.channel.send({ content: `Error: \`Cannot send message to you. please turn on your Dms\`.` });
      }
    });
    if (!errors) {      
